<template>
  <div class="mediaDistribution">
      <div class="spreadPrice spreadMain">
      <!-- 面包屑导航部分 -->
        <el-breadcrumb separator="/">
            <el-breadcrumb-item>当前位置:  </el-breadcrumb-item>
            <el-breadcrumb-item :to="{ path: '/home' }">首页</el-breadcrumb-item>
            <el-breadcrumb-item>媒体发布</el-breadcrumb-item>
            <el-breadcrumb-item><span class="t-text">媒体价格</span></el-breadcrumb-item>
        </el-breadcrumb>
      <el-container>
        <el-header>
            <p type="primary" @click="initMediaType(1)" v-if="query.type==1" class="active">主流媒体</p>
            <p type="primary" @click="initMediaType(1)" v-else>主流媒体</p>
            <p type="primary" @click="initMediaType(2)" v-if="query.type==2" class="active">自媒体</p>
            <p type="primary" @click="initMediaType(2)" v-else>自媒体</p>
            <p type="primary" @click="initMediaType(3)"  v-if="query.type==3" class="active">区块链媒体</p>
            <p type="primary" @click="initMediaType(3)"  v-else>区块链媒体</p>
            <p type="primary" @click="initMediaType(4)"  v-if="query.type==4" class="active">快讯</p>
            <p type="primary" @click="initMediaType(4)" v-else>快讯</p>
        </el-header>    
      </el-container>    
  </div>
    ...
    <el-dialog :visible="dialogShow.submitOrder">
      <div class="pb20">
        ...
        <ft-button
          backgroundColor="#2D68F6"
          @click="
            dialogShow.payment = true;
            dialogShow.submitOrder = false;
          "
          class="mt20"
          >确认发布</ft-button
        >
...
export default Vue.extend({
  ...
  methods: {
    ...
    initMediaType(x:number){
      this.query.type=x;
      this.upArticleParam.type=x;
      this.typeListParam={type:x};
      this.createStrategyParam.type=x;
      this.tableDataParam.type=x;
      this.initData();
    },
    ...
    upArticleMethod(password: string) {
      ...
      articleMapper.upArticle(param).then(
        (result: any) => {
          ...
          this.notifyResult(res.code, res.msg);
          if (res.code == 200) {
            this.$router.push({ name: "mgMediaMedia" });
          }
        },
        (err: CommonIe.ErrParamIe) => {
          this.$message.error(err.msg);
...
.spreadMain{
  .active{
      background-color: #2D68F6 !important;
